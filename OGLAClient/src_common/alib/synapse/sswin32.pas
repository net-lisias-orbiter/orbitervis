...
{$H+}
interface
uses SyncObjs,SysUtils,Classes,Windows;
//############################################################################//
...
IPPROTO_TCP   =6;		//TCP
IPPROTO_UDP   =17;	//ser Datagram Protocol
IPPROTO_IPV6  =41;
IPPROTO_ICMPV6=58;
IPPROTO_RAW   =255;
IPPROTO_MAX   =256;
...
//############################################################################//
var in6addr_any,in6addr_loopback:TInAddr6;
//############################################################################//
...
TGetSockOpt=function(s:TSocket; level,optname:integer; optval:PChar;var optlen:integer):integer;stdcall;
TSendTo=function(s:TSocket; const Buf; len,flags:integer; addrto:PSockAddr;tolen:integer):integer;stdcall;
...
TRecvFrom=function(s:TSocket; var Buf; len,flags:integer; from:PSockAddr;var fromlen:integer):integer;stdcall;
...
TGetAddrInfo=function(NodeName:PChar; ServName:PChar; Hints:PAddrInfo;var Addrinfo:PAddrInfo):integer;stdcall;
TFreeAddrInfo=procedure(ai:PAddrInfo);stdcall;
TGetNameInfo=function(addr:PSockAddr; namelen:integer; host:PChar;hostlen:DWORD; serv:PChar; servlen:DWORD; flags:integer):integer;stdcall;
T__WSAFDIsSet=function (s:TSocket; var FDSet:TFDSet):Bool;stdcall;
TWSAIoctl=function (s:TSocket; dwIoControlCode:DWORD; lpvInBuffer:Pointer;cbInBuffer:DWORD; lpvOutBuffer:Pointer; cbOutBuffer:DWORD;lpcbBytesReturned:PDWORD; lpOverlapped:Pointer;lpCompletionRoutine:pointer):u_int;stdcall;
//############################################################################//
var
...
//############################################################################//
function IN6_IS_ADDR_UNSPECIFIED(const a:PInAddr6):boolean;begin result:=((a^.u6_addr32[0]=0)and(a^.u6_addr32[1]=0) and(a^.u6_addr32[2]=0)and(a^.u6_addr32[3]=0));end;
//############################################################################//
function IN6_IS_ADDR_LOOPBACK(const a:PInAddr6):boolean;
begin
...
//############################################################################//
function IN6_IS_ADDR_LINKLOCAL(const a:PInAddr6):boolean;begin result:=((a^.u6_addr8[0]=$FE)and(a^.u6_addr8[1]=$80));end;
function IN6_IS_ADDR_SITELOCAL(const a:PInAddr6):boolean;begin result:=((a^.u6_addr8[0]=$FE)and(a^.u6_addr8[1]=$C0));end;
function IN6_IS_ADDR_MULTICAST(const a:PInAddr6):boolean;begin result:=(a^.u6_addr8[0]=$FF);end;
function IN6_ADDR_EQUAL(const a:PInAddr6; const b:PInAddr6):boolean;begin result:=(CompareMem(a,b,sizeof(TInAddr6)));end;
procedure SET_IN6_IF_ADDR_ANY (const a:PInAddr6);begin FillChar(a^,sizeof(TInAddr6),0);end;
procedure SET_LOOPBACK_ADDR6 (const a:PInAddr6);begin FillChar(a^,sizeof(TInAddr6),0); a^.u6_addr8[15]:=1;end;
//############################################################################//
procedure FD_CLR(Socket:TSocket; var FDSet:TFDSet);
var I:integer;
begin
...
//############################################################################//
function FD_ISSET(Socket:TSocket; var FDSet:TFDSet):boolean;begin result:=__WSAFDIsSet(Socket,FDSet);end;
//############################################################################//
procedure FD_SET(Socket:TSocket; var FDSet:TFDSet);
begin
...
//############################################################################//
//############################################################################//
function Bind(s:TSocket; const addr:TVarSin):integer;begin result:=ssBind(s,@addr,SizeOfVarSin(addr));end;
function Connect(s:TSocket; const name:TVarSin):integer;begin result:=ssConnect(s,@name,SizeOfVarSin(name));end;
//############################################################################//
function GetSockName(s:TSocket; var name:TVarSin):integer;
var len:integer;
begin
...
//############################################################################//
function Send(s:TSocket; Buf:TMemory; len,flags:integer):integer;begin result:=ssSend(s,Buf^,len,flags);end;
function Recv(s:TSocket; Buf:TMemory; len,flags:integer):integer;begin result:=ssRecv(s,Buf^,len,flags);end;
function SendTo(s:TSocket; Buf:TMemory; len,flags:integer; addrto:TVarSin):integer;begin result:=ssSendTo(s,Buf^,len,flags,@addrto,SizeOfVarSin(addrto));end;
//############################################################################//
function RecvFrom(s:TSocket; Buf:TMemory; len,flags:integer; var from:TVarSin):integer;
var x:integer;
begin
...
begin
 ...
 try
  ...
  if Hints.ai_socktype=SOCK_RAW then begin
   ...
   end else if(IP=cLocalhost) or (IP=c6Localhost)then begin
    result:=synsock.GetAddrInfo(nil,PChar(Port),@Hints,Addr);
   end else begin
    result:=synsock.GetAddrInfo(PChar(IP),PChar(Port),@Hints,Addr);
   end;
  end;
  if result=0 then if(Addr<>nil)then Move(Addr^.ai_addr^,Sin,Addr^.ai_addrlen);
 finally if Assigned(Addr)then synsock.FreeAddrInfo(Addr);end;
end;
...
begin
 ...
 if not IsNewApi(family)then begin
  ...
  try
   ...
   ServEnt:=nil;
   if ProtoEnt<>nil then ServEnt:=synsock.GetServByName(PChar(Port),ProtoEnt^.p_name);
   if ServEnt=nil then Sin.sin_port:=synsock.htons(StrToIntDef(Port,0)) else Sin.sin_port:=ServEnt^.s_port;
   if IP=cBroadcast then Sin.sin_addr.s_addr:=u_long(INADDR_BROADCAST) else begin
    ...
    if Sin.sin_addr.s_addr=u_long(INADDR_NONE)then begin
     ...
     if HostEnt<>nil then Sin.sin_addr.S_addr:=u_long(Pu_long(HostEnt^.h_addr_list^)^);
    end;
   end;
  finally SynSockCS.Leave;end;
 end else begin
  ...
  end else Hints1.ai_family:=Family;
  ...
  result:=r;
  sin:=sin1;
  if r<>0 then if TwoPass then begin
    r:=GetAddr(IP,Port,Hints2,Sin2);
    result:=r;
    if r=0 then sin:=sin2;
   end;
 end;
end;
//############################################################################//
...
begin
 ...
 if not IsNewApi(Sin.AddressFamily)then begin
  ...
  r:=getnameinfo(@sin,SizeOfVarSin(sin),PChar(host),hostlen, PChar(serv),servlen,NI_NUMERICHOST+NI_NUMERICSERV);
  if r=0 then result:=PChar(host);
 end;
end;
//############################################################################//
function GetSinPort(Sin:TVarSin):integer;
begin
 if(Sin.sin_family=AF_INET6)then result:=synsock.ntohs(Sin.sin6_port)
                            else result:=synsock.ntohs(Sin.sin_port);
end;
//############################################################################//
procedure ResolveNameToIP(Name:string; Family,SockProtocol,SockType:integer; const IPList:TStrings);
  ...
  try
   ...
   if r=0 then begin
    ...
    while not(AddrNext=nil)do begin
     if not(((Family=AF_INET6)and(AddrNext^.ai_family=AF_INET)) or ((Family=AF_INET)and(AddrNext^.ai_family=AF_INET6)))then begin
      hostlen:=NI_MAXHOST;
      servlen:=NI_MAXSERV;
      setlength(host,hostlen);
      setlength(serv,servlen);
      r:=getnameinfo(AddrNext^.ai_addr,AddrNext^.ai_addrlen,PChar(host),hostlen,PChar(serv),servlen,NI_NUMERICHOST+NI_NUMERICSERV);
      if r=0 then begin
       host:=PChar(host);
       IPList.Add(host);
      end;
...
begin
 ...
 if not IsNewApi(Family)then begin
  ...
  try
   ...
   ServEnt:=nil;
   if ProtoEnt<>nil then ServEnt:=synsock.GetServByName(PChar(Port),ProtoEnt^.p_name);
   if ServEnt=nil then result:=StrToIntDef(Port,0)else result:=synsock.htons(ServEnt^.s_port);
  finally SynSockCS.Leave;end;
 end else begin
  Addr:=nil;
  try
   ...
   if(r=0) and Assigned(Addr)then begin
    if Addr^.ai_family=AF_INET then result:=synsock.htons(Addr^.ai_addr^.sin_port);
    if Addr^.ai_family=AF_INET6 then result:=synsock.htons(PSockAddrIn6(Addr^.ai_addr)^.sin6_port);
   end;
  finally if Assigned(Addr)then synsock.FreeAddrInfo(Addr);end;
 end;
end;
 ...
 if not IsNewApi(Family)then begin
  ...
  try
   ...
   if(r=0) and Assigned(Addr)then begin
    ...
    setlength(host,hostlen);
    setlength(serv,servlen);
    r:=getnameinfo(Addr^.ai_addr,Addr^.ai_addrlen,PChar(host),hostlen,PChar(serv),servlen,NI_NUMERICSERV);
    if r=0 then result:=PChar(host);
   end;
  finally if Assigned(Addr)then synsock.FreeAddrInfo(Addr);end;
 end;
...
begin
 ...
 if stack='' then stack:=DLLStackName;
 SynSockCS.Enter;
 try
  ...
  if result then Inc(SynSockCount);
 finally SynSockCS.Leave;end;
end;
//############################################################################//
function DestroySocketInterface:boolean;
begin
 ...
 try
  Dec(SynSockCount);
  if SynSockCount<0 then SynSockCount:=0;
  if SynSockCount=0 then begin
   if LibHandle<>0 then begin
